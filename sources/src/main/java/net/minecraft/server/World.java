...
public abstract class World implements IBlockAccess {
    ...
    // Akarin - Re-add asynchronous lighting updates
    public ExecutorService lightingExecutor = Executors.newSingleThreadExecutor(new ThreadFactoryBuilder().setNameFormat("Akarin - Async Lighting Thread").build());
    ...
    private int getRawBlockLightAsync(BlockPosition pos, EnumSkyBlock lightType, Chunk currentChunk, List<Chunk> neighbors) {
    ...
    public void a(int i, int j, int k, int l) {
        ...
        if (this.worldProvider.m()) {
            for (i1 = k; i1 <= l; ++i1) {
                this.updateLight(EnumSkyBlock.SKY, new BlockPosition(i, i1, j)); // Akarin - Re-add asynchronous lighting updates
            }
        }
    ...
    public boolean w(BlockPosition blockposition) {
        ...
        if (this.worldProvider.m()) {
            flag |= this.updateLight(EnumSkyBlock.SKY, blockposition); // Akarin - Re-add asynchronous lighting updates
        }

        flag |= this.updateLight(EnumSkyBlock.BLOCK, blockposition); // Akarin - Re-add asynchronous lighting updates
        return flag;
    }
    ...
    public boolean c(EnumSkyBlock enumskyblock, BlockPosition blockposition, Chunk chunk, List<Chunk> neighbors) { // Akarin
        // CraftBukkit start - Use neighbor cache instead of looking up
        // Chunk chunk = this.getChunkIfLoaded(blockposition.getX() >> 4, blockposition.getZ() >> 4); // Akarin
        if (chunk == null /*|| !chunk.areNeighborsLoaded(1)*/ /*!this.areChunksLoaded(blockposition, 17, false)*/) { // Akarin
            // CraftBukkit end
            return false;
        } else {
            int i = 0;
            ...
            int k = this.getLightForAsync(enumskyblock, blockposition, chunk, neighbors); // Akarin
            int l = this.getRawBlockLightAsync(blockposition, enumskyblock, chunk, neighbors); // Akarin
            ...
            if (l > k) {
                ...
                while (i < j) {
                    ...
                    l2 = this.getLightForAsync(enumskyblock, blockposition1, chunk, neighbors); // Akarin
                    if (l2 == l3) {
                        this.setLightForAsync(enumskyblock, blockposition1, 0, chunk, neighbors); // Akarin
                        if (l3 > 0) {
                            ...
                            if (i3 + j3 + k3 < 17) {
                                ...
                                for (int j4 = 0; j4 < i4; ++j4) {
                                    ...
                                    // Akarin start
                                    final Chunk pooledChunk = this.getLightChunk(blockposition_pooledblockposition, chunk, neighbors);
                                    if (pooledChunk == null) {
                                        continue;
                                    }
                                    int j5 = Math.max(1, pooledChunk.getBlockData(blockposition_pooledblockposition).c()); // OBFHELPER: getLightOpacity
                                    // Akarin end

                                    l2 = this.getLightForAsync(enumskyblock, blockposition_pooledblockposition, chunk, neighbors); // Akarin
            ...
            while (i < j) {
                ...
                int k5 = this.getLightForAsync(enumskyblock, blockposition2, chunk, neighbors); // Akarin

                l2 = this.getRawBlockLightAsync(blockposition2, enumskyblock, chunk, neighbors); // Akarin
                if (l2 != k5) {
                    this.setLightForAsync(enumskyblock, blockposition2, l2, chunk, neighbors); // Akarin
                    if (l2 > k5) {
                        i3 = Math.abs(i2 - i1);
                        j3 = Math.abs(j2 - j1);
                        k3 = Math.abs(k2 - k1);
                        ...
                        if (i3 + j3 + k3 < 17 && flag) {
                            if (this.getLightForAsync(enumskyblock, blockposition2.west(), chunk, neighbors) < l2) {
                                this.J[j++] = i2 - 1 - i1 + 32 + (j2 - j1 + 32 << 6) + (k2 - k1 + 32 << 12);
                            }

                            if (this.getLightForAsync(enumskyblock, blockposition2.east(), chunk, neighbors) < l2) {
                                this.J[j++] = i2 + 1 - i1 + 32 + (j2 - j1 + 32 << 6) + (k2 - k1 + 32 << 12);
                            }

                            if (this.getLightForAsync(enumskyblock, blockposition2.down(), chunk, neighbors) < l2) {
                                this.J[j++] = i2 - i1 + 32 + (j2 - 1 - j1 + 32 << 6) + (k2 - k1 + 32 << 12);
                            }

                            if (this.getLightForAsync(enumskyblock, blockposition2.up(), chunk, neighbors) < l2) {
                                this.J[j++] = i2 - i1 + 32 + (j2 + 1 - j1 + 32 << 6) + (k2 - k1 + 32 << 12);
                            }

                            if (this.getLightForAsync(enumskyblock, blockposition2.north(), chunk, neighbors) < l2) {
                                this.J[j++] = i2 - i1 + 32 + (j2 - j1 + 32 << 6) + (k2 - 1 - k1 + 32 << 12);
                            }

                            if (this.getLightForAsync(enumskyblock, blockposition2.south(), chunk, neighbors) < l2) {
                                this.J[j++] = i2 - i1 + 32 + (j2 - j1 + 32 << 6) + (k2 + 1 - k1 + 32 << 12);
                            }
                        }
                    }
